...
def main():
    ...
    show_dark_image = col3.checkbox("Show Dark Mask")
    if show_dark_image:
        ...
        if uploaded_file is None:
            ...
            sample_output = os.path.join('static', 'sample_output_b.png')
            ...
            if response.status_code == 200:
                ...
                output = os.path.join('static', 'Results','output_b.png')
                image_comparison(
                    img1=input,
                    img2=output,
                    label1="Input_image",
        ...
        if uploaded_file is None:
            ...
            #current_file_path = os.path.abspath(__file__)
            sample_input = os.path.join('static', 'sample_input.png')
            sample_output = os.path.join('static', 'sample_output_w.png')
            # Create two columns for image display
            image_comparison(
                    img1=sample_input,
                    img2=sample_output,
            ...
            if response.status_code == 200:
                response_data = json.loads(response.text)
                input = os.path.join('static', 'Results', 'input.png')
                output = os.path.join('static', 'Results','output_w.png')
                image_comparison(
                    img1=input,
                    img2=output,
                    label1="Input_image",
